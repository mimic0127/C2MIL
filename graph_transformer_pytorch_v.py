...
class Attention(nn.Module):
    ...
    def forward(self, nodes, edges, edge_index, mask=None):
        h = self.heads
        ...
        e_kv = repeat(e_kv, 'b e d -> b h e d', h=h)
        ...
        src, dst = edge_index
        k = k[:, :, src] + e_kv  # Shape: (batch, h, E, d)
        v = v[:, :, src] + e_kv  # Shape: (batch, h, E, d)

        sim = einsum('b h n d, b h e d -> b h e', q[:, :, dst], k) * self.scale
        attn = sim.softmax(dim=-1)

        out = torch.zeros_like(q)
        out.index_add_(2, dst, attn.unsqueeze(-1) * v)
...
class GraphTransformer(nn.Module):
    ...
    def forward(self, nodes, edges=None, edge_index=None, mask=None):
        """
        Args:
            nodes: (batch, N, dim)
            edges: (batch, E, edge_dim)
        ...
        for attn_block, ff_block in self.layers:
            ...
            nodes = attn_residual(attn(nodes, edges, edge_index, mask=mask), nodes)
